...
import fr.umlv.astar.priorityqueue.PrioritizedElement;
import fr.umlv.astar.priorityqueue.PriorityQueue;
...
public class AStar {
	...
	/**
	 * This method search the shortest path in a graph to join a start vertex
	 * and a end vertex. This method perform astar algorithm. The vertices of
	 * generic type V must override equals() and hashCode() methods to be used
	 * in this algorithm.
	 * 
	 * @param graph
	 *            the graph.
	...
	public static <V> AStarResult<V> aStarAlgorithm(Graph<V> graph,
		...
		while (!open.isEmpty()) {
			...
			int numberOfSuccessors = graph
					.numberOfSuccessors(currentVertexValue);
	...
	static class AStarVertex<V> implements PrioritizedElement {
		...
		/**
		 * Get the total cost (actual cost + estimated cost) to join the end
		 * vertex.
		 * 
		 ...
		 * 
		 * @return the predecessor of the current vertex, or null if there is
		 *         not.
		 */
		...
		public boolean equals(Object o) {
			...
			return (this.totalCost == av.totalCost
					&& this.predecessor == av.predecessor && this.value
					.equals(av.value));
		}
		...
		/**
		 * Static method to search in a list the astar vertex which encapsulate
		 * the generic value of type V. If there is no such vertex, this methode
		 * returns null;
		 * 
		 * @param vertices
		 ...
		 * @return the astar vertex which encapsulate the generic value, or
		 *         null.
		 */
